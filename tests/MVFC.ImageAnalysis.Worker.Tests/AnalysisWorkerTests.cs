...
public sealed class AnalysisWorkerTests
{
    ...
    public async Task GetRootShouldReturnOk()
    {
        ...
        (await response.Content.ReadAsStringAsync(TestContext.Current.CancellationToken)).Should().Be("mvfc-image-analysis-worker ok");
    }

    [Fact]
    public void VisionApiClientShouldBeRegistered()
    {
        // Arrange
        var mockMediator = Substitute.For<IMediator>();
        using var factory = new AnalysisWorkerFactory(mockMediator);
        ...
        // Act
        var visionClient = factory.Services.GetService<IVisionApiClient>();

        // Assert
        visionClient.Should().NotBeNull();
    }
    ...
    public async Task PostPubSubPushAndLoggerEnabledShouldLogWarningAndReturnBadRequestWhenRequestIsInvalid()
    {
        ...
        using var factory = new AnalysisWorkerFactory(mockMediator, mockLogger);
